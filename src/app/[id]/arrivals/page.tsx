...
export default function Arrivals() {
    ...
    const journeysFromDB = async (): Promise<Journey[]> => {
        ...
        const request = await fetch(`/api/v1/bahnhof-proxy?id=${station?.evaNr}&type=arrivals&duration=${maxDuration}&locale=${browserLanguage()}`);
    ...
    const updateJourneys = async (appendJourneys: boolean) => {
        ...
        const mappedJourneys = mapConnections(prevJourneys, connections, "arrivals", appendJourneys);
        const sorted = sort(mappedJourneys, "arrivals");
