...
public class Time
{
    ...
    [Column("planned_time")] public required DateTime PlannedTime { get; set; }

    [Column("actual_time")] public required DateTime ActualTime { get; set; }

    [Column("delay")] public required int Delay { get; set; }
    ...
    public static Time CreateTime(JsonElement timeElement, JsonElement trackElement)
    {
        ...
        if (!DateTime.TryParse(timeElement.GetProperty("predicted").GetString() ?? string.Empty, null,
                DateTimeStyles.AdjustToUniversal, out var actualTime))
            actualTime = plannedTime;

        var plannedPlatform = string.Empty;
        if (trackElement.TryGetProperty("prediction", out var plannedPlatformElement) && plannedPlatformElement.ValueKind != JsonValueKind.Null)
            plannedPlatform = plannedPlatformElement.GetString() ?? string.Empty;
        
        string actualPlatform = string.Empty;
        if (trackElement.TryGetProperty("target", out var actualPlatformElement) && actualPlatformElement.ValueKind != JsonValueKind.Null)
