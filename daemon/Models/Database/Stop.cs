...
public class Stop
{
	...
	public static Stop CreateStop(JsonElement element, string journeyId)
	{
		...
		if (element.TryGetProperty("status", out var statusElement) && statusElement.ValueKind != JsonValueKind.Null)
			...
			throw new ArgumentNullException(nameof(element), $"Failed to parse the Stop's cancelled status for {journeyId}");

		var stop = new Stop()
		{
			Cancelled = cancelled,
			EvaNumber = int.Parse(
				element.GetProperty("station").GetProperty("evaNo").GetString()
					?? throw new ArgumentNullException(nameof(element), $"Failed to parse the Stop's evaNumber for {journeyId}")
			),
			Name =
				element.GetProperty("station").GetProperty("name").GetString()
				?? throw new ArgumentNullException(nameof(element), $"Failed to parse the Stop's name for {journeyId}"),
			Messages = element
				.GetProperty("messages")
				.EnumerateArray()
				.Select(message => StopMessage.CreateStopMessage(message, journeyId))
				.ToList(),
		};
		...
		if (element.TryGetProperty("arrivalTime", out var arrivalTimeProp) && arrivalTimeProp.ValueKind != JsonValueKind.Null)
			stop.Arrival = Time.CreateTime(arrivalTimeProp, trackElement);

		if (
			element.TryGetProperty("departureTime", out var departureTimeProp)
