...
public class JourneyMessage : AbstractMessage
{
	...
	public static JourneyMessage CreateJourneyMessage(JsonElement element, string journeyId)
	{
		...
		if (element.TryGetProperty("code", out var codeElement) && codeElement.ValueKind != JsonValueKind.Null)
			code = codeElement.ToString();
		else if (element.TryGetProperty("id", out var idElement) && idElement.ValueKind != JsonValueKind.Null)
			code = idElement.ToString();
		else
			throw new ArgumentNullException(nameof(element), $"Failed to parse the JourneyMessage code for {journeyId}");

		return new()
		{
			Code = code,
			Message =
				element.GetProperty("caption").GetString()
				?? throw new ArgumentNullException(
					nameof(element),
					$"Failed to parse the JourneyMessage message for {journeyId}"
				),
			Summary =
				element.GetProperty("shortText").GetString()
				?? throw new ArgumentNullException(
					nameof(element),
...
public class StopMessage : AbstractMessage
{
	...
	public static StopMessage CreateStopMessage(JsonElement element, string journeyId)
	{
		...
		if (element.TryGetProperty("code", out var codeElement) && codeElement.ValueKind != JsonValueKind.Null)
			...
			throw new ArgumentNullException(nameof(element), $"Failed to parse the StopMessage code for {journeyId}");
