...
public class StationDiscoveryService
{
	...
	private readonly ILogger<StationDiscoveryService> _logger;
	...
	public StationDiscoveryService(ApiService apiService, ILogger<StationDiscoveryService> logger)
	{
	...
	public async Task DiscoverStations(CancellationToken cancellationToken = default)
	{
		...
		await File.WriteAllTextAsync(
			Path.Combine(_tempFolder, "stada.json"),
			JsonSerializer.Serialize(stada, _jsonSerializerOptions),
			Encoding.UTF8,
			cancellationToken
		);
	...
	private async Task<int> DiscoverRisStations(
	...
	private async Task<JsonElement> SaveStations(
		int depth,
		BoundingBox boundingBox,
		GeoPosition center,
		int radius,
	...
	{
		if (
			jsonElement.TryGetProperty("stopPlaces", out var stopPlacesArray)
