...
const loadStation = async (evaNumber: string): Promise<Station> => {
	...
	if (!response.ok) throw error(404, `No station found for ${evaNumber}. Maybe try a different one?`);

	const station = (await response.json()) as Station;
	if (!station.evaNumber) throw error(404, `No station found for ${evaNumber}. Maybe try a different one?`);
	return station;
};
