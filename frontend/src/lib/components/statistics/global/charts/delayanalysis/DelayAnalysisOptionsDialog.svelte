<script lang="ts" module>
	...
	const availableTransportFilters: DelayAnalysisTransportFilter[] = [
		{
			id: "long_distance",
			label: "Long Distance",
			icon: LongDistance,
	...
	export { type ScheduleType, type DelayMode, type DelayAnalysisTransportFilter, availableTransportFilters };
</script>
	...
	<div class="grid grid-cols-2 gap-4">
		...
		<DialogItem title="Series" class="col-span-2">
			<div class="flex flex-wrap gap-2">
				...
				{#each availableTransportTypes as transportType (transportType)}
					{@const transportTypeFilter = availableTransportFilters.find(
						(filter: DelayAnalysisTransportFilter) => filter.transportType === transportType
					)}
		...
		<DialogItem title="Cancellation Behaviour" class="col-span-2 sm:max-w-2xl">
			<p class="text-muted-foreground text-xs leading-relaxed italic">
				...
				departure time. Activating this option will automatically penalize each cancelled stopover with a fixed delay time (the
