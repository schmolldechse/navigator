	...
	const buildCancellationRateChartSeries = (metrics: MetricSeries[]): CancellationRateSeries[] => {
		...
		return buildSeriesDefinitions().map(({ key, label }) => {
			...
			const dataPoints: CancellationRateDataPoint[] = sortedTimestamps.map((timestamp: string) => {
				...
				return { date: DateTime.fromISO(timestamp).toJSDate(), value: cancellationRate, label };
			});
						...
						<div class="flex flex-col gap-y-1">
							<div class="flex items-center justify-between gap-x-4 text-xs">
								<div class="flex items-center gap-x-2">
									<div class="h-1.5 w-1.5 rounded-full" style:background-color={context.series.visibleSeries[0]?.color}></div>
									<span class="text-muted-foreground text-left">{data.label}</span>
								</div>

								<span class="text-text">
									{data.value.toLocaleString(undefined, { maximumFractionDigits: 1 })}{rateMode === "relative" ? "%" : ""}
								</span>
							</div>
						</div>
					{/snippet}
