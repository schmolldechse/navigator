<script module lang="ts">
	...
	const availableTransportFilters: CancellationRateTransportFilter[] = [
		{
			id: "long_distance",
			label: "Long Distance",
			icon: LongDistance,
	...
	export { type ScheduleType, type RateMode, type CancellationRateTransportFilter, availableTransportFilters };
</script>
...
<Dialog
	...
	title="Cancellation Rate Options"
>
			...
			<div class="flex flex-wrap gap-2">
				...
				{#each availableTransportTypes as transportType (transportType)}
					{@const transportTypeFilter = availableTransportFilters.find(
						(filter: CancellationRateTransportFilter) => filter.transportType === transportType
					)}
