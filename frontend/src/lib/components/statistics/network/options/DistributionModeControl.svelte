<script lang="ts">
	...
	type Props = {
		value: NetworkDistributionMode;
		onchange: (mode: NetworkDistributionMode) => void;
	};

	const options: DistributionModeOption[] = [
...
<div role="group" aria-labelledby={`${id}-title`} class="flex min-w-0 flex-col items-start gap-1.5">
	...
	<ToggleGroup
		...
		onselect={(option: DistributionModeOption | undefined) => onchange(option?.value ?? value)}
		class="gap-1"
	>
