<script lang="ts">
	import {
		...
		type BaseMetricDataPointTransportTypeMetricDataPoint,
		type MetricSeries
	} from "@lib/api";
	...
	const getChartSeries = (metrics: MetricSeries[]): TransportTypeSeries[] => {
		...
		return metrics.flatMap((metric: MetricSeries) =>
			metric.dataPoints.map((dataPoint: BaseMetricDataPoint) => {
				const transportTypeDataPoint = dataPoint as BaseMetricDataPointTransportTypeMetricDataPoint;
				return {
