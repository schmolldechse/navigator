...
<script lang="ts" generics="T">
	...
	$effect(() => {
		const index = items.findIndex((item: PillItem<T>) => item.id === selected);
	...
	>
		{#each items as item, index (item.id)}
			<button
				...
				class="pill-button relative z-10 flex cursor-pointer items-center justify-center px-4 py-1 text-sm whitespace-nowrap transition-all duration-300 outline-none"
