<script module lang="ts">
	...
	export type ProjectDimensionTransportFilter = {
		id: string;
		label: string;
		icon: Component<{ class?: string }>;
		transportTypes: TransportType[];
	};
	const availableTransportFilters: ProjectDimensionTransportFilter[] = [
		{
			id: "long_distance",
			label: "Long Distance",
			icon: LongDistance,
...
<script lang="ts">
	...
	let localTransportFilter: TransportType[] = $state([]);

	onMount(
		() =>
			(localTransportFilter = page.url.searchParams
	...
	const handleClose = () => {
		...
		onapply({ start: localTimerange.start, end: localTimerange.end, filter: localTransportFilter });
	};
	...
	const toggleFilterItem = (filter: ProjectDimensionTransportFilter) => {
		const isAllActive = localTransportFilter.length === 0;
		const isFilterActive =
			!isAllActive &&
			filter.transportTypes.every((transportType: TransportType) => localTransportFilter.includes(transportType));

		if (isAllActive) {
			// "Include All" is active → select only this filter
			localTransportFilter = [...filter.transportTypes];
		} else if (isFilterActive) {
			// deselect this filter
			const remaining = localTransportFilter.filter(
				(transportType: TransportType) => !filter.transportTypes.includes(transportType)
			);
			// fall back to "Include All" if no filter is active
			localTransportFilter = remaining.length > 0 ? remaining : [];
		} else localTransportFilter = [...localTransportFilter, ...filter.transportTypes]; // add this filter
	};
</script>
		...
		<div class="divide-muted-foreground/20 flex flex-col divide-y sm:flex-row sm:divide-x sm:divide-y-0">
			...
			<DialogItem title="Transport Types" class="pt-4 sm:pt-0 sm:pl-4">
				<div class="flex flex-wrap gap-2">
					<ToggleStateButton state={localTransportFilter.length === 0} ontoggle={() => (localTransportFilter = [])}>
						Include All
					</ToggleStateButton>

					{#each availableTransportFilters as transportFilter}
						{@const isActive =
							localTransportFilter.length > 0 &&
							transportFilter.transportTypes.every((transportType: TransportType) =>
								localTransportFilter.includes(transportType)
							)}
