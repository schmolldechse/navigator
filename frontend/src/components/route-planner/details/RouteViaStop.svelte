<script lang="ts">
    ...
    const getMessageComponent = (message: RouteMessage, fallbackCancelledValue: boolean): Component => {
        if (message.content === "Stop cancelled" || message.content === "Halt entfällt" || fallbackCancelledValue) return NoOnwardJourney;
        return GeneralWarning;
    }

    const isSamePlatform = (): boolean => {
        return stop.departure!.plannedPlatform === stop.departure!.actualPlatform || (stop.departure!.plannedPlatform === "" && stop.departure!.actualPlatform === "");
    };
</script>
...
{#snippet renderTimeInformation(time: Time)}
	...
	<div class="flex flex-row items-center gap-x-2 min-w-1 basis-1/6">
		<span class={["font-medium text-base", { "hidden": isDelayed }]}>{displayTime(time.plannedTime)}</span>
		{#if isDelayed}
...
<div class="relative flex min-h-fit flex-row py-1">
    ...
    <div class="flex w-[50px] justify-center transition-all duration-500 md:w-[75px]">
        <span class="absolute z-1 h-[15px] w-[15px] border-white/85 border-2 bg-black rounded-full self-center"></span>
		<span class="absolute z-0 h-full w-[4px] bg-white inset-y-0"></span>
	</div>

    <!-- 4/6 Stop -->
    <div class="flex basis-4/6 flex-col leading-none self-center">
    ...
    <!-- 1/6 Platform -->
    <div class="flex flex-col basis-1/6 self-center items-end md:items-start">
        <span class="font-medium text-base">{stop.departure?.plannedPlatform ?? ""}</span>
        {#if !isSamePlatform()}
            <span class="text-background bg-white px-1 text-[10pt] font-bold md:px-2">{stop.departure?.actualPlatform ?? ""}</span>
        {/if}
    </div>
</div>