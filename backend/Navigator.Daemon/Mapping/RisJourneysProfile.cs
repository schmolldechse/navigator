...
public class RisJourneysProfile : Profile
{
    public RisJourneysProfile()
    {
        ...
        CreateMap<RisJourneys.JourneyEvent, JourneyStopPlace>()
            ...
            .ForMember(dest => dest.PlannedTime, opt => opt.MapFrom(src => src.TimeSchedule.UtcDateTime))
            .ForMember(dest => dest.ActualTime, opt => opt.MapFrom(src => src.Time.UtcDateTime))
