...
class UserService {
	...
	async getFavoredStationsBy(userId: string): Promise<typeof StationSchema.static[]> {
		...
		const stationFavors = await database
			.select({
				...
				products: sql<string[]>`COALESCE(array_agg(DISTINCT ${stationProducts.name}) FILTER (WHERE ${stationProducts.name} IS NOT NULL), ARRAY[]::text[])`,
				ril100: sql<string[]>`COALESCE(array_agg(DISTINCT ${stationRil.ril100}) FILTER (WHERE ${stationRil.ril100} IS NOT NULL), ARRAY[]::text[])`
			})
			.from(stations)
	...
	async isStationFavoredBy(userId: string, evaNumber: number): Promise<typeof this.StationFavoredResponse.static> {
		const [existing] = await database.select().from(favoriteStations)
	...
	async favorStationBy(userId: string, evaNumber: number): Promise<typeof this.StationFavoredResponse.static> {
		...
		const stationExists = await database.select().from(stations)
