...
export class BahnhofController {
    ...
    async handleRequest(req: express.Request, res: express.Response): Promise<void> {
        ...
        if (!type) {
            res.status(400).send("Type is required. Expected 'departures' or 'arrivals'");
            return;
        }

        const request = await fetch(`https://bahnhof.de/api/boards/${type}?evaNumbers=${evaNr}&duration=${duration}&locale=${locale}`);
        if (!request.ok) {
            res.status(200);
            return;
        }
...
const mapConnection = (entry: any, type: "departures" | "arrivals"): Connection => {
    const delay: number = calculateDuration(DateTime.fromISO(entry?.scheduledTime), DateTime.fromISO(entry?.realTime), "minutes");
